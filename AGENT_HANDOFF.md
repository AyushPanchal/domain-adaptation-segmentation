...
| 2026-06-02 | Added Kaggle one-experiment workflow | Use `notebooks/kaggle_one_experiment_e01.ipynb` or `scripts/remote/kaggle_run_one_experiment.sh` to run only E01 on Kaggle. Defaults are T4-safe: device 0, batch 8, workers 2, resume auto. The notebook has `RUN_STAGE="smoke"` for 1 epoch first and `RUN_STAGE="full"` for 100 epochs later, streams logs live, prints metrics, and packages a final result zip. |
